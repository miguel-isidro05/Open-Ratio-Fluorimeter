...
class Multichannel90(Measurement):

    NAME = 'Multichannel 90'
    LABEL = constants.CHANNEL_NAMES_90
    UNITS = 'count'
    ...
    @property
    def value(self):
        values = self.sensor_90.raw_values
        self.overflow_indices = tuple(
                index
                for index, value in enumerate(values)
                if value >= self.sensor_90.max_counts
                )
        return tuple(
                constants.OVERFLOW_STR
                if index in self.overflow_indices
                else value
                for index, value in enumerate(values)
                )
    ...
    def create_screen(self):
        from spectral_measurement_screen import SpectralMeasurementScreen

        return SpectralMeasurementScreen()
...
MEASUREMENTS = [RawCount, RelativeUnit, Multichannel90]
